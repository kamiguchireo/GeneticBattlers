...
namespace Engine {
	namespace prefab {
		...
		{
			m_isAvailable = false;
			m_waveFile = m_soundEngine->GetWaveFileBank().FindWaveFile(0, filePath);
			if (!m_waveFile) {
				m_waveFile.reset(new CWaveFile);
				bool result = m_waveFile->Open(filePath);
				if (result == false) {
					//waveファイルの読み込みに失敗。
					m_soundEngine->GetWaveFileBank().UnregistWaveFile(0, m_waveFile);
					m_waveFile.reset();
					return;
				}
				m_waveFile->AllocReadBuffer(m_waveFile->GetSize());	//waveファイルのサイズ分の読み込みバッファを確保する。
				m_soundEngine->GetWaveFileBank().RegistWaveFile(0, m_waveFile);
				unsigned int dummy;
			...
			//サウンドボイスソースを作成。
			m_sourceVoice = m_soundEngine->CreateXAudio2SourceVoice(m_waveFile.get());
